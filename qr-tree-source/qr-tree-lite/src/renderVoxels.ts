...
function project(
  ...
  // smoothstep for seamless corner matching
...
export function renderAnimated(
  ...
  // Pure flat mode: render as 2D grid (same sizing as animated view at tilt=0)
  if (tilt < 0.01) {
...
function drawVoxel(
  ...
  // Transition zone: smooth blend from 2D squares → 3D cubes
  if (tilt < 0.25) {
    const t = tilt / 0.25; // 0→1

    // Draw 2D square base (fades out)
    const flatSx = (x - qrSize / 2) * s + ox;
    const flatSy = (y - qrSize / 2) * s + oy;
    ctx.globalAlpha = 1 - t * t; // quadratic fade-out
    ctx.fillStyle = blendedColor;
    ctx.fillRect(Math.round(flatSx), Math.round(flatSy), Math.ceil(s + 0.5), Math.ceil(s + 0.5));
    ctx.globalAlpha = 1;

    // Draw 3D cube (fades in)
    ...
    ctx.globalAlpha = t * t; // quadratic fade-in
