...
const HOLD_2D = 3000;
const EXPAND = 1500;
const TOTAL = HOLD_3D + COLLAPSE + HOLD_2D + EXPAND; // 7500ms

// Smooth easing that accelerates then decelerates evenly
...
export function IsometricCanvas({ voxels, qrSize, mode, width, height }: Props) {
  ...
  useEffect(() => {
    ...
    function frame(now: number) {
      ...
      if (elapsed < HOLD_3D) {
        // Full 3D — colorful, steady
        tilt = 1;
        grow = 1;
        colorBlend = 1;

      } else if (elapsed < HOLD_3D + COLLAPSE) {
        // 3D → 2D collapse: symmetric easing
        const p = easeInOutCubic((elapsed - HOLD_3D) / COLLAPSE);
        tilt = 1 - p;
        grow = 1 - p;
        // Color stays 1 until tilt < 0.1, then linear fade to B&W
        colorBlend = tilt > 0.1 ? 1 : tilt / 0.1;

      } else if (elapsed < HOLD_3D + COLLAPSE + HOLD_2D) {
        // Flat 2D QR — pure B&W, scannable
        tilt = 0;
        grow = 0;
        colorBlend = 0;

      } else {
        // 2D → 3D expand: mirror of collapse
        const p = easeInOutCubic((elapsed - HOLD_3D - COLLAPSE - HOLD_2D) / EXPAND);
        tilt = p;
        grow = p;
        // Color snaps back at tilt > 0.05 (imperceptible threshold)
